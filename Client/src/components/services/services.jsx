...
const Services = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const data = await getAllServices(); // Fetch services
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service) => (
              ...
              >
                <div className="h-48 overflow-hidden">
                  <img
                    src={service.imageUrl} // Use service.imageUrl from the backend
                    alt={service.name} // Use service.name as the alt text
                    className="w-full h-full object-cover"
                  />
                </div>
                <div className="p-6">
