...
//     try {
//       // 1) Use the shared `login` method
//       const data = await login({ email, password });

//       // 2) Update UI state based on the response
...
//       }, 2000);
//     } catch (error) {
//       // 5) If there's an error, display the error message
...
const Login = () => {
      ...
      try {
        ...
        if (response.user) {
          ...
          if (response.user.role === 'admin') {
            ...
            setTimeout(() => navigate('/admin-dash'), 1500); // 1.5 second delay
          } else {
            navigate('/');
          }
        }
  ...
  const redirectBasedOnRole = (role) => {
    if (role === 'admin') {
      ...
      setTimeout(() => navigate('/'), 1000); // 1 second delay for regular users
    }
  };
